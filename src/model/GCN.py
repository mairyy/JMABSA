...
class GCN(nn.Module):
    def __init__(self, args, in_dim, mem_dim):  
        ...
        self.Wx= nn.Linear(self.attention_heads+self.mem_dim*2, self.attention_heads) 
    ...
    def forward(self, gcn_inputs, attention_mask, aspect_mask, short_mask):
        ...
        attn_tensor = self.attn(gcn_inputs, gcn_inputs, attention_mask, short_mask, aspect_outs)  
        weight_adj=attn_tensor  
        gcn_outputs=gcn_inputs  
        layer_list = [gcn_inputs]

        for i in range(self.layers):
            gcn_outputs = gcn_outputs.unsqueeze(1).expand(gcn_inputs.shape[0], self.attention_heads, gcn_inputs.shape[1], self.mem_dim)   
            ...
            node_outputs1 = gcn_outputs.unsqueeze(1).expand(gcn_inputs.shape[0], gcn_inputs.shape[1], gcn_inputs.shape[1], self.mem_dim)   
...
def attention(query, key, short, aspect, weight_m, bias_m, mask, dropout,):  
    ...
    scores=torch.add(scores, short).to(device=mask.device)
